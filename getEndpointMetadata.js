		...
		try {
			const req = http.request({
				...
				if (res.statusCode != 200) {
					const disconnErr = new Error(`Unexpected response status: ${res.statusCode}`)
			...
			req.on('error', err => {
				const disconnErr = new Error(`Failed reading endpoint metadata: ${err.message}`)
				disconnErr.errType = 'disconnected'
				resolve({err: disconnErr})
			})
			req.end()
...
function onBrowser(host, port) {
	return new Promise((resolve, reject) => {
		try {
			...
			req.onload = function() {
				if (req.readyState !== 4) {
					const disconnErr = new Error(`Unexpected ready state: ${req.readyState}`)
					disconnErr.errType = 'disconnected'
					resolve({err: disconnErr})
					return
				} else if(req.status !== 200) {
					const disconnErr = new Error(`Unexpected response status: ${req.status}`)
					disconnErr.errType = 'disconnected'
					resolve({err: disconnErr})
					return
				}
