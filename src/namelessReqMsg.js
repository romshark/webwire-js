import { Type as MessageType } from './message'
import { New as NewIdentifier } from './identifier'

// NamelessRequestMessage represents a nameless instantiatable webwire request message
// The payload must be a binary Uint8Array
export default function NamelessRequestMessage(type, payload) {
	if (type == null || type < 0 || type > 255) throw new Error(
		`Missing or invalid message type ${type}`
	)
	if (payload != null && !(payload instanceof Uint8Array)) throw new Error(
		`Invalid request payload: ${typeof payload}`
	)
